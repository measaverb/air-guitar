...
try:
    ...
    if guitar_img.shape[2] != 4:
        print("오류: '3.png' 파일에 알파 채널(투명도)이 없습니다.")
    ...
    guitar_img[:, :, 3] = (
        guitar_img[:, :, 3].astype(np.float32) * (opacity / 255.0)
    ).astype(np.uint8)
...
out = cv2.VideoWriter(
    "airguitar_output.mp4", fourcc, 30.0, (int(cap.get(3)), int(cap.get(4)))
)
    ...
    if results.pose_landmarks:
        ...
        if all(lm.visibility > 0.5 for lm in [left_shoulder, right_shoulder]):
            ...
            resized_guitar = cv2.resize(
                guitar_img, (guitar_width, resized_height), interpolation=cv2.INTER_AREA
            )
            ...
            alpha_s = (
                rotated_guitar[overlay_y1:overlay_y2, overlay_x1:overlay_x2, 3] / 255.0
            )
            # 역 알파 채널 (배경용) (1.0 - 투명도)
            alpha_l = 1.0 - alpha_s
            ...
            if alpha_s.shape[0] > 0 and alpha_s.shape[1] > 0:
                for c in range(0, 3):  # B, G, R 3개 채널에 대해 반복
                    ...
                    image[y1_c:y2_c, x1_c:x2_c, c] = (
                        alpha_s
                        * rotated_guitar[
                            overlay_y1:overlay_y2, overlay_x1:overlay_x2, c
                        ]
            ...
            if "guitar_center_x" in locals() and "guitar_center_y" in locals():
                ...
                # 테두리 및 텍스트 표시
                cv2.rectangle(
                    image, (hl_x1, hl_y1), (hl_x2, hl_y2), (0, 220, 0), 2, cv2.LINE_AA
                )
                cv2.putText(
                    image,
                    "ACTIVE ZONE",
            ...
            if results.pose_landmarks:
                ...
                if all(lm.visibility > 0.5 for lm in [right_wrist, right_index]):
                    ...
                    if "prev_tip" not in locals():
                        ...
                        dt = now - prev_time
                        dist_px = math.hypot(
                            fingertip[0] - prev_tip[0], fingertip[1] - prev_tip[1]
                        )
                    ...
                    if "guitar_center_x" in locals() and "guitar_center_y" in locals():
                        ...
                        if (
                            abs(fingertip[0] - guitar_center_x) < rect_half_w
                            and abs(fingertip[1] - guitar_center_y) < rect_half_h
                        ):
                            if velocity > 900:  # px/s 임계값, 환경에 따라 조정
                                cv2.putText(
                                    image,
                                    "STRUM!",
                                    (guitar_center_x - 40, guitar_center_y - 60),
                                    cv2.FONT_HERSHEY_SIMPLEX,
                                    1.0,
                                    (0, 0, 255),
                                    3,
