...
class GuitarRenderer:
    ...
    def __init__(self, image_path, min_conf=0.7):
        ...
        try:
            ...
            opacity = 160
            self.guitar_img_original[:, :, 3] = (
                self.guitar_img_original[:, :, 3].astype(np.float32) * (opacity / 255.0)
            ).astype(np.uint8)
            # 거울 모드에 맞게 미리 좌우 반전
    ...
    def draw_overlay(self, image, pose_results):
        ...
        if pose_results.pose_landmarks:
            ...
            if all(lm.visibility > 0.5 for lm in [left_shoulder, right_shoulder]):
                ...
                cv2.rectangle(
                    overlay,
                    (self.hl_x1, self.hl_y1),
                    (self.hl_x2, self.hl_y2),
                    (0, 255, 0),
                    -1,
                )
...
class AirGuitarApp:
    ...
    def __init__(self, args):
        ...
        self.guitar_renderer = GuitarRenderer(args.guitar_img, args.min_det_conf)
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "--model", default="artifacts/mlp.joblib", help="코드 분류기 모델 경로"
    )
    ap.add_argument(
        "--labels", default="artifacts/labels.json", help="모델 라벨 json 파일 경로"
    )
    ap.add_argument("--guitar_img", default="3.png", help="기타 오버레이 이미지 경로")
    ap.add_argument("--camera", type=int, default=0, help="카메라 인덱스")
    ap.add_argument(
        "--use_z", action="store_true", help="ML 모델이 Z축을 사용했다면 설정"
    )
